...
To fix nomenclature, we will adhere to the definitions given by Handy et. al.~\cite{Handy_DBOC} where the full electron-ion Hamiltonian is written as the sum of the nuclei kinetic energy operator and an electronic Hamiltonian. 
\begin{align}
\hat{H}=\hat{T}_n+\hat{H}_e
\end{align}

The adiabatic assumption will refer to the approximation of the full electron-ion wave function by a single product of an ionic wave function and an eigenstate of the electronic Hamiltonian. The Born-Oppenheimer (BO) approximation will refer to the further neglegence of the diagonal Born-Oppenheimer correction (DBOC) term in solving for the ionic wave function. In this context, the clamped-nuclei ground-state energy $E_c$ is the lowest eigenvalue of the electronic Hamiltonian $\hat{H}_e$ and the non-adiabatic ground-state energy $E_n$ is the eigenvalue of the full Hamiltonian $\hat{H}$. %The BO energy is the sum of the clamped-nuclei energy and zero-point energy (ZPE) of the nuclei estimated without the DBOC term whereas the adiabatic energy includes ZPE with the DBOC term.
...
\begin{figure}[h]
\includegraphics[scale=.37]{Figures/analytical}
\caption{Non-adiabatic contribution to ground state energies of ions and their hydrogen-like atoms calculated with FN-DMC and analytically. $E_n$ denotes the non-adiabatic ground state energy and $E_c$ the clamped-ion ground state energy. Non-adiabatic contribution shows very little dependence with the ionized state. \label{fig:analytical}}
\end{figure}

In Figure \ref{fig:atom-nad-ad}, we demonstrate the non-BO contribution to ground-state energies which is defined as the difference between the non-adiabatic and clamped-ion ground-state energies. By definition, non-BO contribution includes DBOC as well as any non-adiabatic contribution. For atoms and ions, DBOC physically describes the effective confinement of the nucleus due to coupling between the electronic and nuclear motions. This confinement will add a nuclear kinetic contribution to the camped-ion ground-state energy. Non-adiabatic effect encompasses the rest of the modifications to the electron-ion wave function due to electron-nuclear correlation. As shown in Figure \ref{fig:atom-nad-ad}, the amount of non-BO contribution is always positive and increases with atomic number but stays relatively constant between the neatral and cationic species. This observation suggests that for atomic systems, the non-adiabatic coupling between electrons and the nucleus motion is small. Almost all of the non-BO contribution can be attributed to the modification of the nuclear potential energy surface due to the inclusion of the DBOC term. This point is further illustrated by comparing the non-BO contributions in the cations with their corresponding hydrogen-like atoms as shown in Figure \ref{fig:analytical}. The non-BO contributions in the hydrogen-like atoms can be obtained analytically by using the reduced mass. The result in Hartree atomic units is
\begin{align}
E_n-E_c=\frac{Z^2}{2(M+1)}
\end{align}
where $M$ and $Z$ are the the mass and atomic number of the nucleus. We see in Figure \ref{fig:analytical} that even when going from 1 electron to 9 electrons in the case of $F$, the total non-BO energy only changes by 0.5 mHa. It is also interesting to note that for $\text{Li}^{2+}$ and $\text{Be}^{3+}$, the addition of electrons decreases the overall non-BO contribution whereas for $\text{C}^{5+}$ and heavier nuclei, the addition of electrons increase the it.

The ionization energies are reported in Table \ref{tab:ionization} and shown in Figure \ref{fig:ionization}. It is interesting to note that even though the ground-state energy of an atom can differ significantly from the clamped-ion to the non-adiabatic calculation (as much as 3 mHa for $\text{F}^+$). The ionization energies calculated with and without the Born-Oppenheimer approximation are statistically indistinguishable except for O and F, where the differences are 0.26(11)mHa and 0.49(12)mHa respectively.
...
To estimate the non-BO contribution to the ground-state energy of a hydride, we need to calculate the difference between a non-adiabatic and a BO calculation. To produce BO references we add the zero-point energies estimated in~\cite{Feller_Corrections} to our clamped-nuclei results as exact corrections. The results are shown in Figure \ref{fig:dia-nad-ad}. The amount of non-adiabatic contribution for the hydrides investigated all fall in the range of a few mHa and seem to peak for CH and HF. However, it is likely that the actual amount of non-adiabatic energies are smaller than what is being shown in Figure \ref{fig:dia-nad-ad}, as the predicted atomization energies for CH and HF agree less well with experiments than their adiabatic counterparts.
...
\caption{Non-BO contribution to the ground-state energies in hydrides. The BO reference energies are calculated by adding zero-point contributions from \cite{Feller_Corrections} to our clamped-nuclei results. \label{fig:dia-nad-ad}}
\end{figure}
...
\caption{Atomization energies of first row hydrides obtained with FN-DMC relative to experimental data. The BO results are estimated by adding zero-point energies from \cite{Feller_Corrections} to the clamped nuclei energies as exact corrections. \label{fig:atomization}}
\end{figure}
