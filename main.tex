...
In Figure \ref{fig:atom-nad-ad}, we demonstrate the non-adiabatic contribution to ground-state energies. We also provide order-of-magnitude estimates for these contributions by solving the single-electron ions exactly with reduced mass. The result in Hartree atomic units is
\begin{align}
E_n-E_c=\frac{Z^2}{2(M+1)}
\end{align}
where $M$ and $Z$ are the the mass and atomic number of the nucleus. For $\text{Li}^{2+}$ and $\text{Be}^{3+}$, the addition of electrons decreases the overall non-adiabatic contribution whereas for $\text{C}^{5+}$ and heavier nuclei, the addition of electrons increase the it.

The amount of non-adiabatic contribution increases with atomic number but stays relatively constant between the neatral and cationic species. This observation suggests that for atomic systems, the non-adiabatic coupling between the outer-most electron and nucleus motion is small. This point is further illustrated by the ionization energies reported in Table \ref{tab:ionization} and shown in Figure \ref{fig:ionization}. It is interesting to note that even though the ground-state energy of an atom can differ significantly from the clamped-ion to the non-adiabatic calculation (as much as 3 mHa for $\text{F}^+$). The ionization energies calculated with and without the adiabatic assumption are statistically indistinguishable except for O and F, where the differences are 0.26(11)mHa and 0.49(12)mHa respectively.
